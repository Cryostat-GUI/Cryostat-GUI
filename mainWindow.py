...
class mainWindow(QtWidgets.QMainWindow):
    ...
    def load_settings(self):
        ...
        self.window_settings.lineConfFile.setText(self.window_settings.temp_ITC_PIDFile)
    ...
    def initialize_settings(self):
        ...
        self.window_settings.checkUseAuto.toggled["bool"].connect(
            self.settings_temp_ITC_useAutoPID
        )
        ...
        self.sigs = dict(
            ITC=dict(
                useAutocheck=self.sig_ITC_useAutoPID, newFilePID=self.sig_ITC_newFilePID
            ),
    ...
    def y1_changed(self):
        self.plotting_comboValue_Axis_Y1_plot = (
            self.dataplot.comboValue_Axis_Y1.currentText()
        )
    ...
    def plotstart(self):
        print(
            self.plotting_comboValue_Axis_X_plot,
            self.plotting_comboValue_Axis_Y1_plot,
            self.plotting_instrument_for_x,
        )
        array1 = []
        array2 = []
        if self.plotting_instrument_for_x == self.plotting_instrument_for_y1:
            ...
            plt.xlabel(
                self.plotting_comboValue_Axis_X_plot
                + " from table: "
                + str(self.plotting_instrument_for_x)
            )
            plt.ylabel(
                self.plotting_comboValue_Axis_Y1_plot
                + " from table: "
                + str(self.plotting_instrument_for_y1)
            )
    ...
    def run_ILM(self, boolean):
        ...
        if boolean:
            try:
                getInfodata = self.running_thread_control(
                    ILM_Updater(InstrumentAddress=ILM_Instrumentadress),
                    "ILM",
                    "control_ILM",
                )
    ...
    def run_IPS(self, boolean):
        ...
        if boolean:
            try:
                getInfodata = self.running_thread_control(
                    IPS_Updater(InstrumentAddress=IPS_Instrumentadress),
                    "IPS",
                    "control_IPS",
                )
    ...
    def run_LakeShore350(self, boolean):
        ...
        if boolean:
            try:
                getInfodata = self.running_thread_control(
                    LakeShore350_Updater(InstrumentAddress=LakeShore_InstrumentAddress),
                    "LakeShore350",
                ...
                self.LakeShore350_window.spinSetTemp_K.valueChanged.connect(
                    lambda value: self.threads["control_LakeShore350"][
                        0
                    ].gettoset_Temp_K(value)
                )
                ...
                self.LakeShore350_window.comboSetInput_Sensor.activated["int"].connect(
                    lambda value: self.threads["control_LakeShore350"][0].setInput(
                        value + 1
                    )
                )
                ...
                self.LakeShore350_window.checkRamp_Status.toggled["bool"].connect(
                    lambda value: self.threads["control_LakeShore350"][0].setStatusRamp(
                        value
                    )
                )
                ...
                self.LakeShore350_window.spin_threadinterval.valueChanged.connect(
                    lambda value: self.threads["control_LakeShore350"][0].setInterval(
                        value
                    )
                )
    ...
    def run_SR830(self, boolean):
        ...
        if boolean:
            try:
                getInfodata = self.running_thread_control(
                    SR830_Updater(InstrumentAddress=SR830_InstrumentAddress),
                    "SR830",
                    "control_SR830",
                )
