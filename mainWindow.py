   ...
   # ------- Keithley 2182 + Keithley 6221 -------
    ...
    def store_data_Keithley(self, data, dataname, **kwargs):
        ...
        with self.dataLock:
            ...
            if 'GUI_Box' in kwargs:
                try:
                    kwargs['GUI_Box'].activated['str'].connect(lambda value: self.threads[threadname][0].calculate_resistance(self.data['{0:s}'.format(str(value).strip(')').split('(')[1])]['Current_A'], self.data[dataname]['Voltage_V']))
