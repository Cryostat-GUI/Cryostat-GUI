...
while True:
    ...
    sensor1_K = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
    ...
    sensor1_Ohms = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
                id_lakeshore, {"measure_Sensor_Ohm": "Sensor_1_Ohm"}
            )["data_raw"]["Temperature_Ohm"]
        )
    )
    ...
    sensor2_K = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
                id_lakeshore, {"measure_Sensor_K": "Sensor_2_K"}
            )["data_raw"]["Temperature_K"]
        )
    )
    sensor2_Ohms = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
                id_lakeshore, {"measure_Sensor_Ohm": "Sensor_2_Ohm"}
            )["data_raw"]["Temperature_Ohm"]
        )
    )
    ...
    sensor3_K = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
                id_lakeshore, {"measure_Sensor_K": "Sensor_3_K"}
            )["data_raw"]["Temperature_K"]
        )
    )
    sensor3_Ohms = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
                id_lakeshore, {"measure_Sensor_Ohm": "Sensor_3_Ohm"}
            )["data_raw"]["Temperature_Ohm"]
        )
    )
    ...
    sensor4_Ohms = float(
        "{0:.4f}".format(
            zmq_main_control.query_device_command(
                id_lakeshore, {"measure_Sensor_Ohm": "Sensor_4_Ohm"}
            )["data_raw"]["Temperature_Ohm"]
        )
    )

    df_LS350 = df_LS350.append(
        dict(
            datetime=dt,
            sensor1_K=sensor1_K,
            sensor1_Ohms=sensor1_Ohms,
            sensor2_K=sensor2_K,
            sensor2_Ohms=sensor2_Ohms,
            sensor3_K=sensor3_K,
            sensor3_Ohms=sensor3_Ohms,
            sensor4_K=sensor4_K,
            sensor4_Ohms=sensor4_Ohms,
        ),
        ignore_index=True,
    )
