...
class windowSettings(Window_ui):
    ...
    def __init__(
        self, signals, zmqcontext, data, ui_file=".\\configurations\\settings_global.ui"
    ):
        ...
        loadUi(
            ".\\configurations\\window_settings_individualwidgets\\Sequence_tempcontrol_compound.ui",
        ...
        self.checkUseAuto.toggled["bool"].connect(self.ITC_useAutoPID)
        self.lineConfFile.textEdited.connect(self.ITC_PIDFile_store)
        self.pushConfLoad.clicked.connect(self.ITC_PIDFile_send)
        self.lineConfFile.returnPressed.connect(self.ITC_PIDFile_send)
        ...
        tempcontrol.combo_thresholdsLoadingPreset.activated["QString"].connect(
            self.tempcontrol_preset_restore
        )

        tempcontrol.command_sendThresholds.clicked.connect(self.tempcontrol_sendconf)

        self.tempcontrol_preset_parse()
        tempcontrol.pushRefreshPresets.clicked.connect(self.tempcontrol_preset_parse)
        ...
        tempcontrol.spin_Sequence_threshold_slopeResiduals.valueChanged.connect(
            lambda value: self.tempcontrol_conf_store(
                "threshold_slope_residuals", value
            )
        )
    ...
    def ITC_PIDFile_send(self):
        ...
        try:
            ...
            self.show_error_general(
                f"mainthread: settings PIDFile: missing Filename! (TypeError: {e})"
            )

    def tempcontrol_preset_parse(self) -> None:
        settings = QSettings("TUW", "CryostatGUI")
        self.tempcontrol_presets_path = settings.value("Sequence_PresetsPath", str)
        del settings
        os.makedirs(self.tempcontrol_presets_path, exist_ok=True)
        files = [
            os.path.splitext(f)[0]
            for f in os.listdir(self.tempcontrol_presets_path)
            if f.endswith(".json")
            and os.path.isfile(os.path.join(self.tempcontrol_presets_path, f))
        ]
        ...
        self.Sequences_tempcontrol.combo_thresholdsLoadingPreset.addItems(files)
    ...
    def tempcontrol_preset_restore(self, filename: str) -> None:
        ...
        try:
            ...
            self.sig_error.emit(
                f"Settings: The preset file you wanted ({filename}) was not found!"
            )
